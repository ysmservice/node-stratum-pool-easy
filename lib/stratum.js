...
var StratumClient = function(options){
    ...
    function handleMessage(message){
        switch(message.method){
            ...
            case 'eth_submitHashrate':
                handleEthSubmitHashrate(message);
                break;
    ...
    function handleSubscribe(message){
        ...
        // NiceHashのサポート - User-Agentをチェック
        if (message.params && message.params[0]) {
            _this.nicehash = message.params[0].indexOf('NiceHash') !== -1;
        }
        ...
        _this.emit('subscription',
            ...
            function(error, extraNonce1, extraNonce2Size){
                ...
                // NiceHashモードの場合はextraNonce2Sizeを4に設定
                if (_this.nicehash || (_this.passwordOptions && _this.passwordOptions.nicehash)) {
                    extraNonce2Size = 4;
                }
        ...
        function parseWorkerPassword(password) {
            if (!password) return {};
    
            var params = password.split(',');
            var options = {};
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (kv.length === 2) {
                    ...
                    switch(key) {
                        case 'nh':
                            options.nicehash = value === 'true' || value === '1';
                            break;
                        case 'd':
                            var diff = parseFloat(value);
                            if (!isNaN(diff) && diff > 0) options.difficulty = diff;
                            break;
                        case 'proto':
                            var proto = parseInt(value);
                            if (!isNaN(proto)) options.protocol = proto;
                            break;
                    }
                }
            }
        ...
        function handleAuthorize(message, replyToSocket){
            ...
            // パスワードパラメーターを解析
            var passwordOptions = parseWorkerPassword(_this.workerPass);
            _this.passwordOptions = passwordOptions;
    
            // nicehashモードが有効な場合、difficultyを設定
            if (passwordOptions.nicehash && passwordOptions.difficulty) {
                _this.enqueueNextDifficulty(passwordOptions.difficulty);
            }
    
            options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, passwordOptions.password || _this.workerPass, function(result) {
    ...
    function handleEthLogin(message) {
        _this.workerName = message.params[0];
        _this.workerPass = message.params[1] || '';
    ...
    function handleEthSubmitHashrate(message) {
        if (!_this.authorized) {
            sendJson({
                id: message.id,
                error: "unauthorized",
