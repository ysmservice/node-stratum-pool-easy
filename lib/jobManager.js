...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, mixHash){
        ...
        if (options.coin.type === 'ethereum') {
            ...
            var headerHash = job.headerHashBuffer;
            var blockHex;

            // シェアの検証
            if (job.validShare(job.target, nonce, mixHash)) {
                blockHex = job.serializeBlock(mixHash, nonce);
            } else {
            ...
            _this.emit('share', {
                ...
                worker: workerName,
                difficulty: difficulty,
                blockHash: blockHex ? blockHex.blockHash : null
            }, blockHex);

            return {result: true, error: null, blockHash: blockHex ? blockHex.blockHash : null};
        }
        // Equihashの場合
