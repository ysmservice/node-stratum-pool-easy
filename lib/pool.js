...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function OnBlockchainSynced(syncedCallback){
        ...
        var checkSynced = function(displayNotSynced){
            if (['eth','ethash','etchash'].includes(options.coin.algorithm)) {
                _this.daemon.cmd('eth_getWork', [], function(result){
    ...
    // New function to submit ETH blocks
    function SubmitEthBlock(ethParams, callback){
        // ethParams = { nonce, powHash, mixDigest }
        _this.daemon.cmd('eth_submitWork',
            [ethParams.nonce, ethParams.powHash, ethParams.mixDigest],
            function(result){
                // Log result or error as needed
                if (result.error)
                    emitErrorLog('eth_submitWork error: ' + JSON.stringify(result.error));
                else
                    emitLog('Submitted ETH block via eth_submitWork');
                callback();
            }
        );
    }
    ...
    function SetupJobManager(){
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            // If coin is ETH and shareData carries ETH parameters:
            if (['eth', 'ethash', 'etchash'].includes(options.coin.algorithm) && shareData.eth && shareData.blockCandidate){
                // For ETH, blockHex is not provided; submit via eth_submitWork.
                SubmitEthBlock(shareData.eth, function(){
                    emitShare();
                });
            }
    ...
    function DetectCoinData(finishedCallback){
        ...
        if (['eth','ethash','etchash'].includes(options.coin.algorithm)) {
            batchRpcCalls.push(['eth_getWork', []]);
        }
        else {
            batchRpcCalls.push(['submitblock', []]);
        }
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            for (var i = 0; i < results.length; i++){
                ...
                if (rpcCall !== (['eth','ethash','etchash'].includes(options.coin.algorithm) ? 'eth_getWork' : 'submitblock') && (r.error || !r.result)){
            ...
            if (rpcResults[ (['eth','ethash','etchash'].includes(options.coin.algorithm)) ? 'eth_getWork' : 'submitblock' ].message === 'Method not found'){
                options.hasSubmitMethod = false;
            }
            else if (rpcResults[ (['eth','ethash','etchash'].includes(options.coin.algorithm)) ? 'eth_getWork' : 'submitblock' ].code === -1){
    ...
    function StartStratumServer(finishedCallback){
        ...
        _this.stratumServer.on('started', function(){
            ...
            client.on('difficultyChanged', function(diff){
                ...
                if (['eth', 'ethash', 'etchash'].includes(options.coin.algorithm)) {
                    var result = _this.jobManager.processShareETH(
                        params,
                        client.extraNonce1,
                        client.difficulty,
        ...
        if (['eth','ethash','etchash'].includes(options.coin.algorithm)) {
            _this.daemon.cmd('eth_getWork', [],
                function(result){
                    ...
                    // eth_getWork returns an array: [header, seedhash, target]
                    var work = result.response;
                    // Build minimal rpcData for ETH jobs
                    var rpcData = {
                        header: work[0],
                        seedhash: work[1],
                        target: work[2],
                        previousblockhash: '0'.repeat(64), // dummy value
                        bits: work[2],                     // use target as bits fallback
                        curtime: Math.floor(Date.now()/1000),
                        height: 0,
                        transactions: []                   // no transactions for ETH
                    };
