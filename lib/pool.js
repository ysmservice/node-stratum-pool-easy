            ...
            */
            if (!isValidBlock) {
                ...
                if (options.coin.type === 'ethereum') {
                    // Ethereumの場合はJSONRPC経由でwork提出
                    _this.daemon.cmd('eth_submitWork',
                        [blockHex.nonce, blockHex.headerHash, blockHex.mixHash],
                        function(results){
                            var isAccepted = results.every(function(r){
                                return !r.error && r.response === true;
                            });
                            isValidBlock = isAccepted;
                            emitShare();

                            if (isAccepted) {
                                emitLog('Block submitted via eth_submitWork was accepted');
                            } else {
                                emitErrorLog('Block submitted via eth_submitWork was rejected');
                            }
                        }
                    );
                }
