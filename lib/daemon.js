...
function DaemonInterface(daemons, logger){
    ...
    function performHttpRequest(instance, jsonData, callback){
        var options = {
            ...
            headers : {
                'Content-Type': 'application/json', // <-- added for ETH compatibility
    ...
    function cmd(method, params, callback, streamResults, returnRawData){
        if('validateaddress' === method){
            var ethRegex = /^0x[a-fA-F0-9]{40}$/;
            if (ethRegex.test(params[0])){
                return callback({error: null, response: {isvalid: true, address: params[0]}}); // ETH address
